...
public interface DefaultSurveyRepository extends CrudRepository <DefaultSurvey, Long> {
    ...
    default DefaultSurvey findTutorialSurvey() {
        Optional<DefaultSurvey> tutorial = findById(1L);
        if(tutorial.isEmpty()){
            DefaultSurvey tutorialDefaultSurvey = new DefaultSurvey(1L, CourseType.TUTORIAL);
    ...
    default DefaultSurvey findSeminarSurvey() {
        Optional<DefaultSurvey> seminar = findById(1L);
        if(seminar.isEmpty()){
            DefaultSurvey seminarDefaultSurvey = new DefaultSurvey(1L, CourseType.SEMINAR);
    ...
    default DefaultSurvey findPracticalSurvey() {
        Optional<DefaultSurvey> practical = findById(1L);
        if(practical.isEmpty()){
            DefaultSurvey practicalDefaultSurvey = new DefaultSurvey(1L, CourseType.PRACTICAL);
