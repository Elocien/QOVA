...
/**
 * <p>The class responsible for all business logic relating to the mappings in the {@linkplain ResponseController}. </p>
 * <p>Main functionalities are serialising user responses to surveys and compiling results</p>
 */
@Service
@Transactional
public class ResponseManagement {
    ...
    /**
     * Used to parse the {@linkplain CourseType} passed in the URL in String form
     * @param stringType String passed in the URL
     * @return {@linkplain CourseType} enumeration
     */
    ...
    /**
     * Deprecated method that does not function anymore
     * Currently {@linkplain PDFGenerator} is deprecated, but the method technically takes all parameters required to
     * generate a pdf results
     *
     * @param course {@linkplain Course}
     * @param type {@linkplain CourseType}
     * @param groupNumber group No.
     * @param instanceNumber instance No.
     * @return PDF as byte array
     * @throws Exception Writer exception sent by pdf gen
     */
    @Deprecated
    ...
    /**
     * Calculates the number of total submissions some questionnaire has received. Done by getting the total
     * submissions per {@linkplain SurveyResponse}, i.e. per group per instance.
     *
     * @param listOfSurveyResponses The list of all survey responses for some {@linkplain Course}
     * @return Integer with total number of submissions
     */
    ...
    /**
     * <p>The method parses the json generated by the questioneditor. The json
     ...
     * {@linkplain qova.objects.AbstractResponse}. </p>
     *
     * <p>TLDR: Creates a list of sublists each containing {@linkplain qova.objects.AbstractResponse}.
     * These are passed to the {@linkplain qova.objects.SurveyResponse}'s created with the
     * {@linkplain ResponseManagement#createSurveyResponse(JSONArray, Course, CourseType)} method</p>
     *
     ...
     * <br>
     * [{"type": STRING, "default": BOOLEAN, "question": STRING, "options": [STRING, STRING, ...], "answers": [STRING, STRING]}
     * <br>
     * <br>
    ...
    /**
     * Used to retrieve {@linkplain SurveyResponse}'s, given the following parameters:
     *
     * @param course {@linkplain Course}
     * @param type {@linkplain CourseType}
     * @param groupNumber The group number. This can either be an Integer which is parsed from the string, or it can be
     *                    "all" in which case the relevant {@linkplain SurveyResponseRepository} method is used.
     * @param instanceNumber The instance number. This can either be an Integer which is parsed from the string, or it can be
     *      *                    "all" in which case the relevant {@linkplain SurveyResponseRepository} method is used.
     * @return A List of {@linkplain SurveyResponse}s
     */
    ...
    /**
     * Persists user responses to questionnaires
     *
     * @param surveyResponse The relevant {@linkplain SurveyResponse} to which the student submits
     * @param studentResponseJson The JSON generated in the frontend by Javascript, containing the submission
     */
    public void submitStudentResponse(SurveyResponse surveyResponse, JSONArray studentResponseJson) {
        ...
        for (int surveyPosition = 0; surveyPosition < surveyResponse.getListOfResponses().size(); surveyPosition++) {
            ...
            if (abstractResponse instanceof qova.objects.TextResponse) {
                ...
                //Makes sure that text longer than 100,000 chars is discarded
                if(studentText.length() >= 100000){
                    studentText = "";
                }
